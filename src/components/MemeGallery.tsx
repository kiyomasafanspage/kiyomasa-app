...
const memes = [
  ...
  { src: '/assets/event/kiyomasa chalenge.jpg', alt: 'Kiyomasa Challenge' },
];
  ...
  return (
    <section ref={ref} className="relative section-padding">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <span className="text-xs tracking-[0.4em] text-[#ffd700]/60 uppercase font-medium">The Movement</span>
          <h2 className="text-4xl md:text-6xl font-black mt-3 mb-4">
            Meme <span className="gradient-text">Gallery</span>
          </h2>
          <p className="text-white/50 text-sm">Community-created content. Real culture. Real power.</p>
        </motion.div>
          ...
          {memes.map((m, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                <span className="text-xs text-white/80 font-medium">{m.alt}</span>
              </div>
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                style={{ background: 'radial-gradient(ellipse at center, rgba(255,45,45,0.15) 0%, transparent 70%)' }} />
            </motion.div>
          ))}
        </div>
        ...
        {selected && (
          ...
          >
            <div className="relative max-w-2xl w-full aspect-square rounded-2xl overflow-hidden neon-border-gold">
              <Image src={selected} alt="Selected meme" fill className="object-contain" />
            </div>
            <button
              onClick={() => setSelected(null)}
